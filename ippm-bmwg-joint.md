...
In general, the WG expects an implementation for Standards Track documents. In some cases an implementation proof might be not needed. Whenever such an exception applies, the exception justification must be included in the specification document or its write-up.
...
| Jan 2027 | Submit On-Path Telemetry YANG Data Model to the IESG                                                                    | draft-ietf-ippm-on-path-telemetry-yang      | Standards Track |
